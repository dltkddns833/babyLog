...
function getActivityColor(type: string): string {
  if (type.startsWith("breast")) return "text-indigo-600";
  if (type === "formula") return "text-amber-600";
  if (type === "pumped") return "text-blue-600";
  if (type === "diaper") return "text-yellow-600";
  if (type === "night_sleep" || type === "nap") return "text-purple-600";
...
export default function FeedingTable({ data, activities }: Props) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-3 sm:p-5">
      <h3 className="text-base sm:text-lg font-semibold mb-3 sm:mb-4">일별 수유 요약</h3>
      <div className="-mx-1">
        {/* 헤더 */}
        <div className="grid grid-cols-[2fr_1fr_1fr_1fr] sm:grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr] border-b border-gray-200 text-gray-500 text-[11px] sm:text-xs px-1 py-2">
        ...
        {data.map((d) => {
          ...
          return (
            <div key={d.date}>
              <div
                className={`grid grid-cols-[2fr_1fr_1fr_1fr] sm:grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr] px-1 py-1.5 text-[11px] sm:text-xs cursor-pointer transition-colors border-b border-gray-100 ${
              ...
              >
                ...
                <span className="hidden sm:block text-center text-gray-500">{getDaysOld(d.date)}일</span>
                <span className="text-center font-medium">{totalCount}회</span>
                <span className="text-center">{d.feedingTotalMinutes}분</span>
                <span className="text-center text-gray-500">{d.bottleSummary}</span>
                <span className="hidden sm:block text-center text-gray-500">{formatInterval(d.avgFeedingInterval)}</span>
              </div>

              {isExpanded && (
                <div className="bg-gray-50 border-b border-gray-200 py-2 overflow-x-auto">
                  <div className="px-3 min-w-[340px] space-y-1">
                    {dayActivities.map((a, i) => (
                      ...
                      >
                        ...
                        <span className={`font-medium shrink-0 ${getActivityColor(a.type)}`}>
                          {getActivityLabel(a.type)}
                        </span>
                        <span className="text-gray-500 shrink-0">{getActivityDetail(a)}</span>
                        {a.memo && (
                          <span className="inline-block px-1.5 py-0.5 rounded bg-indigo-100 text-indigo-700 text-[10px] sm:text-[11px] font-medium shrink-0">
                            {a.memo}
                          </span>
                        )}
                      </div>
